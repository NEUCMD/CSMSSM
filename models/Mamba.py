...
class Model(nn.Module):   
    def __init__(self, configs):
        ...
        self.mamba = nn.ModuleList(
                    [
                        nn.ModuleDict(
                            dict(
                                mixer=Mamba2(configs),
                                norm=RMSNorm(configs.d_model),
                            )
                        )
    ...
    def forecast(self, x_enc, x_mark_enc):
        ...
        if h is None:
            h = [None for _ in range(self.args.e_layers)]
...
class InferenceCache(NamedTuple):
    ...
    @staticmethod
    def alloc(batch_size: int, args, device: Device = None):
...
class Mamba2(nn.Module):
    def __init__(self, args, device: Device = None):
        super().__init__()
    
        args.d_inner = args.expand * args.d_model
        assert args.d_inner % args.headdim == 0
        args.nheads = args.d_inner // args.headdim
        if args.vocab_size % args.pad_vocab_size_multiple != 0:
            args.vocab_size += (
                args.pad_vocab_size_multiple
                - args.vocab_size % args.pad_vocab_size_multiple
            )
